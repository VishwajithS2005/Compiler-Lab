...
void printParsingTable()
{
    ...
    int stateW = w[0];
    int actionW = 0;
    for (int i = 0; i < (int)termCols.size(); i++)
        actionW += w[1 + i] + 3;
    actionW -= 1;
    int gotoW = 0;
    for (int i = 0; i < (int)ntCols.size(); i++)
        gotoW += w[1 + (int)termCols.size() + i] + 3;
    gotoW -= 1;
